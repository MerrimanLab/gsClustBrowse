...
```{sql, connection = con}
CREATE VIEW IF NOT EXISTS combined AS SELECT marker_info.name, marker_info.chr, marker_info.position, sample.samplecode, batch.*, intensities.* FROM intensities 
...
update_sample_table <- function(gs_long_combined, batch){
  ...
  samples <- gs_long_combined %>% select(samplecode = sample) %>% mutate(batchid = batch, sex = NA, ancestry = NA) %>% distinct()
...
header_spec <- def_col_data_type(here::here("data/test10000_batch1.txt"))

#test <- read_delim(file = here::here("data/test10000_batch1.txt"),delim = '\t', col_types = header_spec$cols,  n_max = 100) 

read_delim_chunked(file = here::here("data/test10000_batch1.txt"),delim = '\t', col_types = header_spec$cols, chunk_size = 2000, callback = SideEffectChunkCallback$new(f), progress = TRUE) 

f <- function(gs_tab, pos){
  process_chunk(gs_tab, batch = 2)
}

header_spec <- def_col_data_type(here::here("data/test10000_batch2.txt"))

#test <- read_delim(file = here::here("data/test10000_batch1.txt"),delim = '\t', col_types = header_spec$cols,  n_max = 100) 

read_delim_chunked(file = here::here("data/test10000_batch2.txt"),delim = '\t', col_types = header_spec$cols, chunk_size = 1000, callback = SideEffectChunkCallback$new(f), progress = TRUE)
```
