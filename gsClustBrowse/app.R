...
pops <- tbl(con, "sample") %>% select( ancestry_specific) %>%  mutate(ancestry_specific = ifelse(is.na(ancestry_specific),"Missing", ancestry_specific)) %>%  pull(ancestry_specific) %>% unique()
    ...
    sidebarPanel(
        tabsetPanel(
            ...
            tabPanel("Plot options",
                     ...
                     radioButtons("facet_options", label = "Facet By", choices = list(None = "none", Ancestry = "ancestry_specific", `Reported Sex` = "reported_sex", `Genetic Sex` = "genetic_sex")),
                     radioButtons("colour_options", label = "Colour", choices = list(Gtype = "gtype", `Reported Sex` = "reported_sex", `Genetic Sex` = "genetic_sex", Ancestry = "ancestry_specific")   )
                     , width = 3)
        )
    ),
...
server <- function(input, output) {
    ...
    filtered_data <- reactive({
        ...
        if (!is.null(input$ancestry_filter)) {
            out_dat <- out_dat %>% mutate(ancestry_specific = ifelse(is.na(ancestry_specific),"Missing", ancestry_specific)) %>% filter(ancestry_specific %in% !!input$ancestry_filter)
        }
